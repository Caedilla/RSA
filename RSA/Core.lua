...
function RSA.AnnouncementCheck() -- Checks against user settings to see if we are allowed to announce.
	...
	if RSA.db.profile.General.GlobalAnnouncements.InWarMode and UnitIsPVP("player") == 1 and InInstance == false and InstanceType == "none" and not LFParty and not LFRaid then return true end -- Enable in World PvP.
	...
	if RSA.db.profile.General.GlobalAnnouncements.InWorld and InstanceType == "none" and not LFRaid then return true end -- Enable in World PvE.
	return false
end
